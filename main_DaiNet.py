...
class DaiNet(nn.Module): # 我们定义网络时一般是继承的torch.nn.Module创建新的子类
    def __init__(self):
        ...
        self.dp = nn.Dropout(0.5)
        #self.conv3 = nn.Conv2d(24, 32, 3)    # 同样是卷积层 
    ...
    def forward(self, x):
        ...
        # fully connect
        x = x.view(-1, 24 * 6 * 6)  # .view( )是一个tensor的方法，使得tensor改变size但是元素的总数是不变的。
...
netname = 'DaiNet'
writer = SummaryWriter(comment='DaiNet_train') # 提供一个 comment 参数，将使用 runs/日期时间-comment 路径来保存日志
writer_total = SummaryWriter(comment='DaiNet_test')

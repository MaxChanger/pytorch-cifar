...
# 定义卷积神经网络
class DaiNet1(nn.Module): # 我们定义网络时一般是继承的torch.nn.Module创建新的子类
    def __init__(self):
        super(DaiNet1, self).__init__()      # 第二、三行都是python类继承的基本操作,此写法应该是python2.7的继承格式,但python3里写这个好像也可以
...
netname = 'DaiNet1'
writer = SummaryWriter(comment='DaiNet1_train') # 提供一个 comment 参数，将使用 runs/日期时间-comment 路径来保存日志
writer_total = SummaryWriter(comment='DaiNet1_test')

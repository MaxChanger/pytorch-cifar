...
# 定义卷积神经网络
class DaiNet4(nn.Module): # 我们定义网络时一般是继承的torch.nn.Module创建新的子类
    def __init__(self):
        super(DaiNet4, self).__init__()      # 第二、三行都是python类继承的基本操作,此写法应该是python2.7的继承格式,但python3里写这个好像也可以
        ...
        self.fc1 = nn.Linear(16 * 4 * 4, 128)   # 接着三个全连接层 Linear(in_features, out_features, bias=True)
        self.fc2 = nn.Linear(128, 64)           #                      输入样本的大小 输出样本的大小 若设置为False这层不学习偏置 默认True
    ...
    def forward(self, x):
        ...
        # fully connect
        x = x.view(-1, 16 * 4 * 4)  # .view( )是一个tensor的方法，使得tensor改变size但是元素的总数是不变的。
...
netname = 'DaiNet4'
writer_train = SummaryWriter(comment='DaiNet4_train') # 提供一个 comment 参数，将使用 runs/日期时间-comment 路径来保存日志
writer_test = SummaryWriter(comment='DaiNet4_test')

...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=1204, shuffle=True, num_workers=2)

testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=1204, shuffle=False, num_workers=2)
...
# 定义卷积神经网络
class DaiNet7(nn.Module): # 我们定义网络时一般是继承的torch.nn.Module创建新的子类
    def __init__(self):
        super(DaiNet7, self).__init__()      # 第二、三行都是python类继承的基本操作,此写法应该是python2.7的继承格式,但python3里写这个好像也可以
        ...
        #self.fc2 = nn.Linear(128, 10) #                      输入样本的大小 输出样本的大小 若设置为False这层不学习偏置 默认True
...
netname = 'DaiNet7'
writer_train = SummaryWriter(comment='DaiNet7_train') # 提供一个 comment 参数，将使用 runs/日期时间-comment 路径来保存日志
writer_test = SummaryWriter(comment='DaiNet7_test')

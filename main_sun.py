...
class Net(nn.Module): # 我们定义网络时一般是继承的torch.nn.Module创建新的子类
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 6, 5)     # 添加第一个卷积层,调用了nn里面的Conv2d
                                            # 输入是3通道的图像，输出是6通道，也就是6个卷积核，卷积核是5*5，其余参数都是用的默认值
        self.pool = nn.MaxPool2d(2, 2)      # 最大池化层  
                                            #2x2采样， MaxPool2d(窗口大小, 窗口移动的步长, padding=0, 一个控制窗口中元素步幅的参数=1, )
        self.conv2 = nn.Conv2d(6, 16, 5)    # 同样是卷积层 
        self.fc1 = nn.Linear(16 * 5 * 5, 120)   # 接着三个全连接层 Linear(in_features, out_features, bias=True)
        self.fc2 = nn.Linear(120, 84)           #                      输入样本的大小 输出样本的大小 若设置为False这层不学习偏置 默认True
...
netname = 'SunNet'
writer = SummaryWriter(comment='SunNet_train') # 提供一个 comment 参数，将使用 runs/日期时间-comment 路径来保存日志
writer_total = SummaryWriter(comment='SunNet_test')
